...
router.get('/', function(req, res, next){
  let headerToken = req.headers['x-access-token']
    if(headerToken){
      ...
      .leftOuterJoin('friends', 'dashboard.user_id', 'friends.friend_id')
      .select('dashboard.username','dashboard.bio','dashboard.name','photos.image_url','dashboard.user_id', 'friends.friend_id')
      .then(data=>{
        res.send(data)
      })
    } else {
